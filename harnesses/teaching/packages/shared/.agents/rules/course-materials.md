...
- **`main`** is student-facing. Treat anything that lands there as public to
  the class.
...
- Prefer the course’s existing structure (`docs/modules/`, `lectures/`,
  `assignments/` when those exist). Do not invent a second layout or run
  `symkit --scaffold` on a tree that already has a course image.
